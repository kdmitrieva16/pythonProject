...
def get_data():
    ...
    for i in list_file:
        ...
        os_prod_list.append(','.join(re.findall(list_template[0], input_data)))
        os_name_list.append(','.join(re.findall(list_template[1], input_data)))
        os_code_list.append(','.join(re.findall(list_template[2], input_data)))
        os_type_list.append(','.join(re.findall(list_template[3], input_data)))
    main_data = [
        ['Изготовитель системы', 'Название ОС', 'Код продукта', 'Тип системы'],
        [], [], []]
...
def write_to_csv(file_name):
    with open(file_name, 'w') as file_out:
        write_csv = csv.writer(file_out, delimiter=',')
        data = get_data()
        for i in data:
